...
namespace Microsoft.Framework.Caching.Memory
{
    public class CompactTests
    {
        ...
        {
            var cache = CreateCache();
            cache.Set("key1", "Value1", new CacheEntryOptions().SetPriority(CacheItemPriority.NeverRemove));
            cache.Set("key2", "Value2", new CacheEntryOptions().SetPriority(CacheItemPriority.NeverRemove));
        ...
        {
            var cache = CreateCache();
            cache.Set("key1", "Value1", new CacheEntryOptions().SetPriority(CacheItemPriority.Low));
            cache.Set("key2", "Value2", new CacheEntryOptions().SetPriority(CacheItemPriority.Low));
            cache.Set("key3", "value3");
            cache.Set("key4", "value4");
            Assert.Equal(4, cache.Count);
            cache.Compact(0.5);
